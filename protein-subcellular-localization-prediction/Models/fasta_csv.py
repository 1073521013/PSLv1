...
# NLS signals list
with open('../datasets/NLS_signals.csv', "r") as f:
    reader = csv.reader(f)
...
def main():
	...
	# Loading train files
	train_file_names = ['cytoplasmic', 'extracellular', 'nuclear', 'plasma_membrane',"ER","lysosomal",                  
                     "Golgi","chloroplast","peroxisomal","mitochondrial","vacuolar"]
	train_dfs = []

	for file_name in train_file_names:
		print('Loading file %s...' % file_name)
		fasta_df = fasta_to_pandas('../datasets/dataset0/'+file_name+'.fasta')
	...
	blind_test = fasta_to_pandas('../datasets/dataset0/blind.fasta')
	print("#-------- Done.")
