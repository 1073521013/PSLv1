...
* train.data_cache.cache_dir (str)
    - make sure to change "/home/guest_majian/python/gcForest_official/gcforest/mnist/fg-tree500-depth100-3folds/datas" to your own path
...
* Fine Grained Scanning:  'train_fg.py'
* Cascade Training: 'train_cascade.py'
...
* First Train the Fine Grained Forest:
    - Run `python train_fg.py --model models/mnist/gcforest/fg-tree500-depth100-3folds.json --log_dir logs/gcforest/mnist/fg --save_outputs`
    - This means:
...
* Then, train the cascade forest (Note: make sure you run the train_fg.py first)
    - run `python train_cascade.py --model models/mnist/gcforest/fg-tree500-depth100-3folds-ca.json`
    - This means:
...
* You could also training a Cascade Forest without fine-grained scanning(but the accuracy will be much lower):
    - Run `python train_cascade.py --model models/mnist/gcforest/ca-tree500-n4x2-3folds.json --log_dir logs/gcforest/mnist/ca`
